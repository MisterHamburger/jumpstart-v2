...
- **As of 2026-05-06:** Pirate Ship → OPEX, Venmo → `PAYROLL_SOURCING` (separated from `PAYROLL` so operations labor is distinguishable from sourcing-team labor). The change accompanied a Kickstart `true_cost` simplification — see "Kickstart COGS" below. CSV import normalizes Co-pilot label "Payroll - Sourcing Fees" → `PAYROLL_SOURCING`. Dashboard RPC aggregates: OPEX = `category='OPEX'`; PAYROLL = `category='PAYROLL'`; PAYROLL_SOURCING = `category='PAYROLL_SOURCING'`; "Kickstart sourcing" aggregates `category IN ('SOURCING','INVENTORY')` minus shipping descriptions minus Jumpstart load vendors (`%smartlots%`, `%inmar%`, `%remarketing%`, `%jumpstart%`, `%boutiquebythebox%`, `%nusource%`, `%quince%`) — those are canonical in the `loads` table, and counting them here double-counted cashflow (fixed 2026-06-10). ⚠️ This aggregate is a **leftover bucket, not an identified set**: anything the filter doesn't recognize silently defaults to "Kickstart sourcing." When a new Jumpstart load vendor appears in bank imports, add it to the exclusion list in `get_dashboard_summary` or cashflow double-counts. ⚠️ Watch for **vendor aliases** — one vendor can post under two descriptions: Inmar-DHL appears both as `... - Inmar-dhl` and as `Dhl Remarketing Liquidation` (its ACH DBA, containing no "inmar"), which leaked $47,812 in 2026 before `%remarketing%` was added (fixed 2026-09-07).
...
### Dashboard RPC
- `get_dashboard_summary(date_cutoff, date_end)` — Payroll = `category = 'PAYROLL'`; Sourcing = `category IN ('SOURCING', 'INVENTORY')` excluding UPS/Pirate Ship and Jumpstart load vendors (smartlots/inmar/remarketing/jumpstart/boutiquebythebox/nusource/quince). Jumpstart inventory cash comes from the `loads` table (`load_cost` only — `load_freight` is always 0 since 2026-06-10: freight is inside Inmar load payments or arrives as separate OPEX from the freight broker). The dashboard's Net Profit/Expenses/Cashflow include `payroll_sourcing`.
...
- **Pool loads / custom brands:** Unmanifested loads are "pools" — at sale time an item is scanned with a barcode TOKEN and COGS resolves to that pool's point-in-time cumulative WAC (`SUM(total_cost)/SUM(quantity)` over loads dated ≤ show date with `quantity > 0`). The token lives in `loads.pool_tag`: RDM→`'RDM'`, UJC (`kind='unmanifested'`)→`'UJC'`, and `kind='custom'` brand loads (Quince, Buck Mason, …)→uppercased brand (e.g. `'QUINCE'`). The `profitability` view's `pool_wac` CTE groups by `pool_tag` and joins `pool_tag = scan barcode`; guards use `barcode NOT IN (SELECT pool_tag FROM loads ...)`. Add custom brand loads via Inputs → Add New Load (kind "Custom brand"); the scanner's Manual Scan pool dropdown and the inventory pools are data-driven from `loads.pool_tag`. A NULL-quantity pool load (e.g. cashflow-only `BBB-1`) is excluded from WAC but still counts in `load_cost`. `loads.landed` (boolean) marks physical arrival: new loads default to in-transit (`false`), toggled to `true` on arrival via Add New Load's status control or the loads-list "Mark landed" button. The Inventory tab reports **landed** pool units as "In stock / Landed Stock Value" and includes in-transit only in the "owned" totals; manifested inventory is treated as fully depleted (in-stock = pools only). ⚠️ A new brand whose bank payment imports as INVENTORY must also be added to the `get_dashboard_summary` sourcing-exclusion list (see Dashboard RPC) or cashflow double-counts.
