-- Fix three profitability view regressions discovered by Admin > Data Integrity:
--
-- 0. IS_TEST FILTER (regression)
--    add-is-test-to-shows.sql introduced `is_test` on shows and filtered
--    out test shows in the live-sales branches. tune-whatnot-fees.sql and
--    add-jumpstart-wac-and-rdm.sql dropped that filter, so the TEST -
--    Kickstart show (id 52, date 2099-12-31) is now leaking into the view.
--    Symptom: Month Additivity exploded to 887 buckets because the test
--    show's far-future date became maxDate. Restoring the filter here.
--
-- 1. BUNDLE REVENUE SPLIT (regression)
--    A prior migration (fix-bundle-shipping-in-profitability.sql) had baked
--    shipping_charged into per-item buyer_paid and shipping_cost into per-item
--    cost_freight for both JS and KS bundle branches. tune-whatnot-fees.sql
--    and add-jumpstart-wac-and-rdm.sql rebuilt the view from scratch and
--    dropped that fix. Symptom: items_sum != sale_price + shipping_charged
--    for every sold bundle box. Restoring it here.
--      buyer_paid     = (sale_price + shipping_charged) / cnt
--      cost_freight   = item_cogs + (shipping_cost / cnt)
--
-- 2. RDM BUNDLE SALES (missing branch)
--    rdm_bundle_sales records bulk Jumpstart RDM sales sold outside Whatnot
--    (e.g., entire crate to a single buyer). Each row has quantity + sale_price
--    but no individual barcodes. The view had no branch for it, so all of that
--    revenue and cost was silently missing from the dashboard. Adds a fifth
--    UNION branch that expands each row into `quantity` virtual items at
--    sale_price/quantity each, with the RDM hardcode cost ($3.41) per item.
--
-- Cost resolution for Jumpstart live sales unchanged:
--   RDM hardcode → m.cost_freight → jumpstart_show_wac.avg_cost
--
-- Run in Supabase Dashboard > SQL Editor
-- Date: 2026-05-11

-- Step 0: shows.is_test was defined in migrations/add-is-test-to-shows.sql but
-- the column was never actually added to the live DB. Add it idempotently
-- and tag the known test shows so the view filter below works.
ALTER TABLE shows ADD COLUMN IF NOT EXISTS is_test BOOLEAN DEFAULT false;
UPDATE shows SET is_test = true WHERE id IN (52, 66) AND COALESCE(is_test, false) = false;
...
-- buyer_paid   = (sale_price + shipping_charged) / cnt   (shipping is revenue)
-- cost_freight = item_cogs + shipping_cost/cnt           (shipping is cost)
SELECT
  bs.id AS scan_id,
  bs.barcode,
...
-- Same shipping treatment as Jumpstart bundles.
SELECT
  bs.id AS scan_id,
  ki.upc AS barcode,
  ('B' || bb.box_number)::text AS listing_number,
...
-- Each rdm_bundle_sales row -> `quantity` virtual items.
-- buyer_paid   = sale_price / quantity
-- cost_freight = $3.41 (RDM hardcode)
SELECT
  -- Synthetic scan_id: needs to be unique numeric across the view's int8 scan_id.
  -- Use a large offset (10^12) plus quantity-index to avoid collisions with
  -- jumpstart_sold_scans.id / jumpstart_bundle_scans.id (both bigserials).
  ...
  3.41::numeric AS cost,
  3.41::numeric AS cost_freight,
  NULL::text AS zone,
  ...
  ROUND(r.sale_price / NULLIF(r.quantity, 0)::numeric - 3.41, 2) AS profit,
  CASE
    WHEN r.sale_price > 0 THEN
      ROUND(
        ((r.sale_price / NULLIF(r.quantity, 0)::numeric - 3.41)
