-- Part 2b: bulk lot sales (Cindy flow) for any pool — J.Crew/Madewell + Damages.
--
-- Background (Josh, 2026-07-22): the old "RDM" bundle-sale tab sold whole crates
-- off-Whatnot into rdm_bundle_sales, costed at the RDM pool WAC. RDM is now blended
-- into JCM, and we also bulk-sell the Damages bucket. Generalize: add
-- rdm_bundle_sales.pool_tag (default 'RDM' for history) and cost each bundle sale
-- at ITS pool's landed WAC. JCM sells like a normal Jumpstart lot (profit + inventory
-- adjusted); DAMAGE sells at ~$1 (near break-even — the real loss was already taken
-- via shrinkage at tag time). Inventory depletion by pool_tag is in the app.
--
-- CREATE OR REPLACE (RPC untouched). No-op for existing rows (all default pool_tag
-- 'RDM', still costed at RDM WAC). Run in Supabase Dashboard > SQL Editor.
-- Date: 2026-07-22

ALTER TABLE rdm_bundle_sales ADD COLUMN IF NOT EXISTS pool_tag TEXT NOT NULL DEFAULT 'RDM';
