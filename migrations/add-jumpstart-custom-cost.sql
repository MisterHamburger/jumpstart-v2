-- Adds Jumpstart custom COGS support to the profitability view, mirroring the
-- Kickstart pattern from add-custom-item-cost.sql. Use case: unmanifested
-- J.Crew/Madewell batches that aren't RDMs — set a per-show or per-item custom
-- cost and bad-barcode scans in that show will pick it up.
--
...
--     → si.custom_item_cost             (per-item override by sticker #)
--     → sh.custom_item_cost             (per-show custom COGS)
--     → jwac.avg_cost                   (WAC fallback)
--
-- Flag changes for the Jumpstart branch:
--   is_bad_barcode: true only when nothing resolved (same contract as before,
--                   but now also false when a custom cost applied)
--   is_wac_cost:    fell through to WAC (not RDM, no manifest, no custom cost)
--   is_custom_cost: fell through to per-item or per-show custom cost
--
-- The shows.custom_item_cost and show_items.custom_item_cost columns already
-- exist (added by add-custom-item-cost.sql) and are shared across channels.
--
...
-- Cost order: RDM hardcode → m.cost_freight → si.custom_item_cost
--             → sh.custom_item_cost → jwac.avg_cost
SELECT
  s.id AS scan_id,
  s.barcode,
  ...
  (s.barcode IS DISTINCT FROM 'RDM'
  ...
  false AS is_bundle,
  -- is_wac_cost: fell through to show-WAC (not RDM, not in manifest, no custom cost)
  (s.barcode IS DISTINCT FROM 'RDM'
    AND m.barcode IS NULL
    AND si.custom_item_cost IS NULL
    AND sh.custom_item_cost IS NULL
...
-- Recreate dashboard summary function (unchanged — just drops & re-adds so the
-- view rebuild doesn't leave a broken dependency).
