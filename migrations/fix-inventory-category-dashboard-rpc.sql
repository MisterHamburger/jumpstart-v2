-- Fix get_dashboard_summary() for INVENTORY category change
-- Venmo payments moved from PAYROLL → INVENTORY category
-- Kickstart sourcing now includes: SOURCING + INVENTORY categories
-- Removed separate Venmo query (no longer in PAYROLL)
...
BEGIN
  ...
  -- Sum payroll expenses (full amount for P&L — no longer includes Venmo)
  SELECT COALESCE(SUM(amount), 0)
  INTO total_payroll
  FROM expenses
  ...
  -- Kickstart sourcing: SOURCING category + INVENTORY category (Venmo moved here)
  -- Excludes shipping (UPS, Pirate Ship)
  SELECT COALESCE(SUM(amount), 0)
  INTO total_sourcing
  FROM expenses
