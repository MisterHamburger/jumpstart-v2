-- Damage write-down (shrinkage) support: let negative-quantity adjustment
-- loads affect the pool WAC.
--
-- Background (Josh, 2026-07-16): tagging damages removes units from the
-- J.Crew/Madewell pool but leaves their unrecovered cost behind, so the WAC
-- RISES (shrinkage). That's done with a dated adjustment load (quantity -N,
-- total_cost -N at $1/unit recovery). The pool_wac CTE previously required
-- quantity > 0, which excluded those adjustments. Relaxing to `<> 0` lets them
-- lower the pool's effective quantity/cost and raise the WAC.
--
-- No-op for all current data: no load has a non-positive quantity today, and
-- NULL-quantity loads (BBB-1) are still excluded by `IS NOT NULL`. Verify the
-- profitability channel totals are byte-identical before/after.
--
-- CREATE OR REPLACE (not DROP) so get_dashboard_summary is left untouched.
