-- FIX: Kickstart profitability uses true_cost (includes tax, shipping, sourcing)
-- instead of raw bin price (cost). Also adds trigger to auto-calculate true_cost
-- on every INSERT/UPDATE so this never breaks again.
-- Run in Supabase Dashboard > SQL Editor
-- Date: 2026-03-11

-- ============================================================
-- PART 1: Column defaults so new items always get fee rates
-- ============================================================
ALTER TABLE kickstart_intake
  ALTER COLUMN sales_tax_rate SET DEFAULT 0.10,
  ALTER COLUMN shipping_fee SET DEFAULT 1.50,
  ALTER COLUMN sourcing_fee SET DEFAULT 1.00;

-- ============================================================
-- PART 2: Trigger to auto-calculate true_cost on INSERT/UPDATE
-- Formula: cost * (1 + sales_tax_rate) + shipping_fee + sourcing_fee
-- ============================================================
CREATE OR REPLACE FUNCTION calculate_kickstart_true_cost()
RETURNS TRIGGER AS $$
BEGIN
  NEW.sales_tax_rate := COALESCE(NEW.sales_tax_rate, 0.10);
  NEW.shipping_fee := COALESCE(NEW.shipping_fee, 1.50);
  NEW.sourcing_fee := COALESCE(NEW.sourcing_fee, 1.00);
  NEW.true_cost := ROUND(
    COALESCE(NEW.cost, 0) * (1 + NEW.sales_tax_rate)
    + NEW.shipping_fee
    + NEW.sourcing_fee,
    2
  );
...
DROP TRIGGER IF EXISTS calculate_true_cost_trigger ON kickstart_intake;
CREATE TRIGGER calculate_true_cost_trigger
  BEFORE INSERT OR UPDATE ON kickstart_intake
  FOR EACH ROW
  EXECUTE FUNCTION calculate_kickstart_true_cost();

-- ============================================================
-- PART 3: Profitability view — use true_cost for all Kickstart COGS
-- ============================================================
DROP VIEW IF EXISTS profitability CASCADE;

CREATE VIEW profitability AS

-- CTE: per-show average TRUE cost for Kickstart items (WAC fallback for unmatched items)
...
-- KICKSTART: Whatnot live sales — USES true_cost (includes tax, shipping, sourcing)
SELECT
  s.id AS scan_id,
  s.barcode,
  si.listing_number::text AS listing_number,
...
-- KICKSTART BUNDLES: sold outside Whatnot (0% fees) — USES true_cost
SELECT
  bs.id AS scan_id,
  ki.upc AS barcode,
  ('B' || bb.box_number)::text AS listing_number,
  ...
  ki.msrp,
  COALESCE(ki.true_cost, ki.cost) AS cost,
...
-- ============================================================
-- PART 4: Recreate dashboard summary function (depends on profitability view)
-- ============================================================
