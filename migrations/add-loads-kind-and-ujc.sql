-- Adds loads.kind column and unifies cost handling for "unmanifested" load
-- pools (RDM and direct-from-J.Crew batches without per-unit barcodes).
--
-- Replaces the hardcoded $3.41/unit RDM cost in the profitability view with a
-- point-in-time weighted-average cost (WAC) of all loads of that kind dated
-- on or before the show date. Adds a parallel 'UJC' barcode literal for
-- unmanifested-J.Crew items, costed against their own WAC pool.
--
-- Why WAC instead of hardcoded:
--   • RDM cost varies slightly per load; running WAC tracks the real basis.
--   • Same mechanism handles any future unmanifested kind without code edits.
--   • Point-in-time: a new load added today does NOT shift the cost of past
--     sales — only sales on or after the new load's date pick up the blend.
--
-- Existing RDM loads' total_cost is overwritten to (quantity × $3.41) so the
-- WAC of all current RDM loads still resolves to $3.41/unit on day one. The
-- next RDM load entered with its true total_cost blends with that baseline.
--
-- Run in Supabase Dashboard > SQL Editor
-- Date: 2026-05-28

-- ============================================================================
-- PART 1: loads.kind column + backfill
-- ============================================================================

ALTER TABLE loads ADD COLUMN IF NOT EXISTS kind TEXT NOT NULL DEFAULT 'manifested';

-- CHECK constraint added separately so re-running the ALTER above is safe.
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'loads' AND constraint_name = 'loads_kind_check'
  ) THEN
    ALTER TABLE loads ADD CONSTRAINT loads_kind_check
      CHECK (kind IN ('manifested', 'rdm', 'unmanifested'));
  END IF;
END $$;

-- Tag existing RDM rows. Matches the pre-migration heuristic in
-- src/components/JumpstartInventory.jsx (vendor or notes containing 'rdm').
UPDATE loads
SET kind = 'rdm'
WHERE (vendor ILIKE '%rdm%' OR notes ILIKE '%rdm%')
  AND kind = 'manifested';

-- Pre-migration audit: surface RDM rows before total_cost is overwritten so
-- you can sanity-check the WAC baseline in the SQL Editor output.
DO $$
DECLARE r RECORD;
BEGIN
  RAISE NOTICE 'Existing RDM loads (pre-migration total_cost overwrite):';
  FOR r IN
    SELECT id, date, quantity, total_cost,
           CASE WHEN COALESCE(quantity, 0) > 0
                THEN ROUND(total_cost / quantity, 4)
                ELSE NULL END AS implied_unit_cost
    FROM loads
    WHERE kind = 'rdm'
    ORDER BY date NULLS LAST, id
  LOOP
    RAISE NOTICE '  Load % | date=% | qty=% | total_cost=% | implied_unit=%',
      r.id, r.date, r.quantity, r.total_cost, r.implied_unit_cost;
  END LOOP;
END $$;

-- Overwrite RDM total_cost so quantity × 3.41 = total_cost. Historical
-- effective COGS was $3.41/unit (via the prior hardcode in the view), so this
-- preserves that basis through the new WAC formula.
UPDATE loads
SET total_cost = ROUND(quantity * 3.41, 2)
WHERE kind = 'rdm'
  AND quantity IS NOT NULL
  AND quantity > 0;

-- ============================================================================
-- PART 2: profitability view rewrite
-- Replaces RDM hardcode with point-in-time WAC, adds parallel 'UJC' branch.
-- ============================================================================
...
jumpstart_show_wac AS (
  -- Per-show WAC fallback for genuinely bad barcodes (not RDM, not UJC).
...
-- Point-in-time WAC for unmanifested load kinds ('rdm', 'unmanifested').
-- For each (kind, date_of_interest) we compute SUM(total_cost)/SUM(quantity)
-- across all loads of that kind with date <= date_of_interest. The set of
-- dates includes every show date and bundle-sold date referenced by the rest
-- of the view, so any cost-resolution branch can look up its date and get
-- the WAC that was in effect at that moment.
...
-- WAC replaces RDM hardcode here too; UJC parallel branch added.
SELECT
  bs.id AS scan_id,
  bs.barcode,
...
-- Cost = RDM WAC at sold_at date (was hardcoded $3.41).
SELECT
...
-- ============================================================================
-- PART 3: dashboard summary RPC
-- Merges PAYROLL_SOURCING split (add-payroll-sourcing-to-dashboard-rpc.sql,
-- 2026-05-06) with manual_revenue folding (add-manual-revenue.sql, 2026-05-15)
-- since DROP VIEW ... CASCADE drops the function and we need to restore it.
-- ============================================================================
...
BEGIN
  ...
  SELECT COALESCE(SUM(amount), 0) INTO total_payroll
    FROM expenses
    WHERE category = 'PAYROLL'
      AND date >= expense_start
      AND (date_end IS NULL OR date <= date_end);

  -- Sourcing-team labor (Venmo)
  SELECT COALESCE(SUM(amount), 0) INTO total_payroll_sourcing
    FROM expenses
  ...
  SELECT COALESCE(SUM(total_cost), 0) INTO total_load_cost
    FROM loads
    WHERE date >= effective_start
      AND (date_end IS NULL OR date <= date_end);

  SELECT COALESCE(SUM(freight_per_item * quantity), 0) INTO total_load_freight
    FROM loads
    WHERE date >= effective_start
      AND (date_end IS NULL OR date <= date_end);

  -- Kickstart sourcing: direct vendor payments only
  SELECT COALESCE(SUM(amount), 0) INTO total_sourcing
    FROM expenses
