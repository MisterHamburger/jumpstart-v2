-- Tune Whatnot fee estimate in profitability view to match reality.
--
-- Reconciled against Whatnot's March 2026 financial statement:
--   Sales (hammer total):   $94,603.80
--   Commission fees:         $6,810.34   →  7.20% of hammer  (Premier Shop)
--   Payment processing:      $4,848.29   →  5.12% of hammer
--   Total seller-side fees: 12.32%
--
-- Old formula:  7.2% commission + 2.9% + $0.30/item   → ~10.5%, under-estimated fees
-- New formula:  7.2% commission + 5.1% processing     → ~12.3%, tracks reality ± ~0.5%
--
-- The 5.1% processing rate is higher than Whatnot's published 2.9% because
-- Whatnot calculates payment processing on buyer TOTAL (hammer + shipping + tax),
-- which runs ~1.75× the hammer. 2.9% × 1.75 ≈ 5.1%.
--
-- Applies to Jumpstart and Kickstart Whatnot live-sales branches only.
-- Bundle branches already use 0% fees (sold outside Whatnot).
--
...
-- Fee formula: 7.2% commission + 5.1% payment processing (no per-item fixed fee)
SELECT
  s.id AS scan_id,
  s.barcode,
  ...
  ROUND(
    si.buyer_paid - si.buyer_paid * 0.123 - COALESCE(m.cost_freight, 0),
    2
  ) AS profit,
  CASE
...
-- Same fee formula as Jumpstart: 7.2% commission + 5.1% payment processing.
-- COALESCE chain for unknown items: per-item → per-show → WAC.
SELECT
  s.id AS scan_id,
  s.barcode,
  si.listing_number::text AS listing_number,
...
-- KICKSTART BUNDLES: sold outside Whatnot (0% fees) — unchanged
SELECT
  bs.id AS scan_id,
  ki.upc AS barcode,
  ('B' || bb.box_number)::text AS listing_number,
...
-- Recreate dashboard summary function (depended on the old view)
