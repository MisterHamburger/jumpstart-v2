...
-- RDM (Returned & Damaged) items have barcode='RDM' and fixed $3.41 COGS
SELECT
  s.id AS scan_id,
  s.barcode,
  ...
  CASE WHEN s.barcode = 'RDM' THEN 3.41 ELSE m.cost END AS cost,
  CASE WHEN s.barcode = 'RDM' THEN 3.41 ELSE m.cost_freight END AS cost_freight,
  ...
  ROUND(
    ...
    - CASE WHEN s.barcode = 'RDM' THEN 3.41 ELSE COALESCE(m.cost_freight, 0) END,
    2
  ) AS profit,
  CASE
