-- FIX: Kickstart bundle profitability — Box 1 hardcoded cost
...
-- Box 1: hardcoded cost (689.50 total) — original true_cost data was incorrect
-- Box 2+: uses true_cost from kickstart_intake, sale_price for revenue
SELECT
  bs.id AS scan_id,
  ki.upc AS barcode,
  ('B' || bb.box_number)::text AS listing_number,
  ...
  ki.msrp,
  CASE
    WHEN bb.box_number = 1 THEN ROUND(689.50 / NULLIF(item_counts.cnt, 0), 6)
    ELSE COALESCE(ki.true_cost, ki.cost, 0)
  END AS cost,
  CASE
    WHEN bb.box_number = 1 THEN ROUND(689.50 / NULLIF(item_counts.cnt, 0), 6)
    ELSE COALESCE(ki.true_cost, ki.cost, 0)
