-- Cost each pool load from when it LANDED, not when it was paid.
--
-- Background (Josh, 2026-08-17): marking Loads 25 & 27 landed today (paid-dated
-- 08-06) re-blended the JCM WAC backward to 08-06 and retroactively recosted
-- ~4,379 J.Crew/Madewell items on shows from 08-06 onward ($7.28 -> $9.16),
-- knocking ~$8.2k off gross profit — even though those shows sold BEFORE the
-- loads physically arrived. A load should only affect the WAC for shows on/after
-- it actually landed (when we can sell it).
--
-- Fix: add loads.landed_at (the day a load is marked landed) and use
-- COALESCE(landed_at, date) as the pool_wac cutoff. Historical landed loads have
-- landed_at = NULL and fall back to their paid date (behavior UNCHANGED, no
-- history disruption). Only Loads 25 & 27 get a real landed_at (today), so they
-- stop costing the 08-06..08-16 shows and correctly start on 08-17.
-- Going forward the app stamps landed_at whenever a load is marked landed.
--
-- CREATE OR REPLACE (RPC untouched). Run in Supabase Dashboard > SQL Editor.
-- Date: 2026-08-17

ALTER TABLE loads ADD COLUMN IF NOT EXISTS landed_at DATE;

-- Loads 25 & 27 physically arrived today (were paid-dated 08-06).
UPDATE loads SET landed_at = '2026-08-17' WHERE id IN ('Load 25', 'Load 27');
...
pool_wac AS (
  ...
  JOIN loads l
   ...
   AND COALESCE(l.landed_at, l.date) <= d.d
...
SELECT
  ...
  r.pool_tag::text AS barcode,
  (r.pool_tag || '-' || r.id::text) AS listing_number,
  (r.pool_tag || ' Bundle - ' || COALESCE(r.buyer_name, 'Unknown')) AS show_name,
  r.sold_at::date AS show_date,
  NULL::text AS time_of_day,
  'Jumpstart' AS channel,
  (r.pool_tag || ' bundle item')::text AS description,
  NULL::text AS category,
  NULL::numeric AS msrp,
  COALESCE(pw.avg_cost, CASE WHEN r.pool_tag = 'RDM' THEN 3.41 ELSE 0 END)::numeric AS cost,
  COALESCE(pw.avg_cost, CASE WHEN r.pool_tag = 'RDM' THEN 3.41 ELSE 0 END)::numeric AS cost_freight,
  NULL::text AS zone,
  (r.pool_tag || ' bundle item')::text AS product_name,
  ...
  ROUND(r.sale_price / NULLIF(r.quantity, 0)::numeric - COALESCE(pw.avg_cost, CASE WHEN r.pool_tag = 'RDM' THEN 3.41 ELSE 0 END), 2) AS profit,
  CASE
    WHEN r.sale_price > 0 THEN
      ROUND(
        ((r.sale_price / NULLIF(r.quantity, 0)::numeric - COALESCE(pw.avg_cost, CASE WHEN r.pool_tag = 'RDM' THEN 3.41 ELSE 0 END))
...
LEFT JOIN pool_wac pw ON pw.pool_tag = r.pool_tag AND pw.for_date = r.sold_at::date
